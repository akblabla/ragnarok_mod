...
function NewCombat:getDamage(attacker, defender, solveType, isCounter, attackerPos, defenderPos, attackerPath, defenderPath, isGroove, grooveWeaponIdOverride)
	...
	if not (isGroundUnit and defender.unitClass.isStructure) then
		if not isGroove and (not Stats.isRanged(effectiveAttacker.unitClassId) and not Wargroove.canStandAt(effectiveAttacker.unitClassId, defender.pos)) then
			return nil, false
		end
	end
	if Ragnarok.hasCrown(effectiveAttacker) and not isGroove then return nil, false end
	...
	if StealthManager.isActive(defender.playerId) then
