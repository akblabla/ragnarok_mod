...
local HospitalReinforce = Verb:new()
...
local function getSurroundingCities(unit, pos)
    ...
    for i, p in ipairs(Wargroove.getTargetsInRangeAfterMove(unit, pos, pos, 1, "unit")) do
        ...
        if city and city.unitClass.canReinforce and city.unitClassId == "hospital" then
...
local function getReinforceMultiplierAt(unit, pos)
    local reinforceMultiplier = 1.0
    for i, city in ipairs(getSurroundingCities(unit, pos)) do
        reinforceMultiplier = math.min(reinforceMultiplier, city.unitClass.reinforceMultiplier)
    end
    return reinforceMultiplier
end
...
function HospitalReinforce:getMaximumRange(unit, endPos)
    return 1
end


function HospitalReinforce:getTargetType()
    return "unit"
end


function HospitalReinforce:canExecuteAnywhere(unit)
...
function HospitalReinforce:canExecuteAt(unit, endPos)
...
function HospitalReinforce:canExecuteWithTarget(unit, endPos, targetPos, strParam)
...
function HospitalReinforce:getHealAndCost(unit, targetPos)
    local desiredHealing = maxUnitHealth - unit.health
    local availableHealing = getHealingAvailableAt(unit, targetPos)
    local affordableHealing = getAffordableHealing(unit)
    local reinforceMultiplier = getReinforceMultiplierAt(unit, targetPos)

    local toHeal = math.min(math.min(desiredHealing, availableHealing), affordableHealing)
    local cost = getHealingCost(toHeal, unit) * reinforceMultiplier

    return toHeal, cost
end


function HospitalReinforce:getCostAt(unit, endPos, targetPos)
    local toHeal, cost = self:getHealAndCost(unit, endPos)
    return math.floor(cost)
end


function HospitalReinforce:execute(unit, targetPos, strParam, path)
    local targetUnit = Wargroove.getUnitAt(targetPos)       
    ...
    Wargroove.logAnalyticsAction("UnitReinforced", unit.playerId, unit.unitClassId, tostring(toHeal))
end

function HospitalReinforce:generateOrders(unitId, canMove)
    local orders = {}
...
function HospitalReinforce:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
    local unitClass = Wargroove.getUnitClass(unit.unitClassId)
