...
function SpellHex:canExecuteAnywhere(unit)
    return Wargroove.getMoney(unit.playerId) >= self:calculateSpellCost(unit)
end

function SpellHex:calculateSpellCost(unit)
    local discount = 1.0

    if Wargroove.isConquestMode() then 
        discount = unit.unitClass.verbCostMultiplier

        -- In conquest mode, witches don't need money to cast hex.
...
function SpellHex:getCostAt(unit, endPos, targetPos)
    return self:calculateSpellCost(unit)
end


function SpellHex:execute(unit, targetPos, strParam, path)
    Wargroove.changeMoney(unit.playerId, -self:calculateSpellCost(unit))
    ...
    for i, pos in ipairs(targets) do
        ...
        if u ~= nil then
            ...
            if Wargroove.areEnemies(u.playerId, unit.playerId) and (not uc.isStructure) then
                local adjustedDamage = spellDamage * u.damageTakenPercent
    ...
    Wargroove.logAnalyticsAction("UnitAbility", unit.playerId, "hex", "")
end
