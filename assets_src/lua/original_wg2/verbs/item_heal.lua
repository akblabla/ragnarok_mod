...
local ItemHeal = Verb:new()
local spellCost = 150
local healAmount = 30
local costScoreFactor = 0.5


function ItemHeal:getMaximumRange(unit, endPos)
    return 0
end


function ItemHeal:getTargetType()
    return "all"
end


function ItemHeal:canExecuteAnywhere(unit)
    return Wargroove.getMoney(unit.playerId) >= spellCost
end


function ItemHeal:getCostAt(unit, endPos, targetPos)
    return spellCost
end


function ItemHeal:execute(unit, targetPos, strParam, path)
...
function ItemHeal:generateOrders(unitId, canMove)
    local orders = {}
...
function ItemHeal:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
