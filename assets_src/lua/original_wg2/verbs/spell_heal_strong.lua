...
local SpellHealStrong = Verb:new()
local spellCost = 300
local healAmount = 30
local costScoreFactor = 0.5

local discountTable = {
    { blessingId="blessing09", discount=0.5 }
}

function SpellHealStrong:getMaximumRange(unit, endPos)
    return 2
end


function SpellHealStrong:getTargetType()
    return "all"
end


function SpellHealStrong:canExecuteAnywhere(unit)
    return Wargroove.getMoney(unit.playerId) >= SpellHealStrong:calculateSpellCost(unit)
end

function SpellHealStrong:calculateSpellCost(unit)
    local discount = 1.0

    for _, d in ipairs(discountTable) do
        if Wargroove.doesUnitHaveBlessing(unit, d.blessingId) then
            discount = discount * d.discount
        end
    end

    return spellCost * discount
end


function SpellHealStrong:getCostAt(unit, endPos, targetPos)
    return SpellHealStrong:calculateSpellCost(unit)
end


function SpellHealStrong:execute(unit, targetPos, strParam, path)
    Wargroove.changeMoney(unit.playerId, -SpellHealStrong:calculateSpellCost(unit))
    ...
    Wargroove.logAnalyticsAction("UnitAbility", unit.playerId, "heal", "")
end


function SpellHealStrong:generateOrders(unitId, canMove)
    local orders = {}
...
function SpellHealStrong:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
