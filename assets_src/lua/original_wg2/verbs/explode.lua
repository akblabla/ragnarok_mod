...
function Explode:getMaximumRange(unit, endPos)
    return 0
end

function Explode:getTargetType()
    return "unit"
end
...
function Explode:execute(unit, targetPos, strParam, path)
    ...
    for i, pos in ipairs(Wargroove.getTargetsInRange(unit.pos, 1, "unit")) do
        ...
        if u and Wargroove.areEnemies(u.playerId, unit.playerId) then
            ...
            local damage = Combat:getGrooveAttackerDamage(unit, u, "average", unit.pos, u.pos, path, "kojiAttack") * 0.5
