...
local ExplodeStrong = Verb:new()

local knockonDamage = 20

function ExplodeStrong:getMaximumRange(unit, endPos)
    return 0
end

function ExplodeStrong:getTargetType()
    return "unit"
end

function ExplodeStrong:getTargetArrows(unit, targetPos, endPos)
    local results = {}
   
    for i, pos in ipairs(Wargroove.getTargetsInRange(targetPos, 1, "unit")) do
        local u = Wargroove.getUnitAt(pos)
        if u then
            local pushResults = Wargroove.getPushPullResult(targetPos, pos, 1, true, false)
...
function ExplodeStrong:execute(unit, targetPos, strParam, path)
    ...
    for i, pos in ipairs(Wargroove.getTargetsInRange(unit.pos, 1, "unit")) do
        ...
        if u then
            ...
            local pushResults = Wargroove.getPushPullResult(targetPos, pos, 1, false, false)
...
function ExplodeStrong:generateOrders(unitId, canMove)
    local orders = {}
...
function ExplodeStrong:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
