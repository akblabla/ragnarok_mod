...
function ItemHex:getMaximumRange(unit, endPos)
    return 0
end


function ItemHex:getTargetType()
    return "all"
end


function ItemHex:canExecuteAnywhere(unit)
    return Wargroove.getMoney(unit.playerId) >= spellCost
end


function ItemHex:getCostAt(unit, endPos, targetPos)
    return spellCost
end


function ItemHex:execute(unit, targetPos, strParam, path)
...
function ItemHex:generateOrders(unitId, canMove)
    local orders = {}
...
function ItemHex:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
    local unitClass = Wargroove.getUnitClass(unit.unitClassId)
