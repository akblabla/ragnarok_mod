...
function Pull:getMaximumRange(unit, endPos)
    return pullRange
end

function Pull:getTargetType()
    return "all"
end

function Pull:getTargetArrows(unit, targetPos, endPos)
    local results = {}
...
function Pull:canExecuteWithTarget(unit, endPos, targetPos, strParam)
...
function Pull:execute(unit, targetPos, strParam, path)
...
function Pull:generateOrders(unitId, canMove)
    local orders = {}
...
function Pull:getScore(unitId, order)
    local unit = Wargroove.getUnitById(unitId)
    local unitClass = Wargroove.getUnitClass(unit.unitClassId)
