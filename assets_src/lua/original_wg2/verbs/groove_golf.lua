...
function Golf:isInCone(unitPos, movingUnitPos, targetPos, tier)
  ...
  if (unitPos.x == movingUnitPos.x) then
    if absXDiff > absYDiff or absYDiff > maxDist[tier] or absXDiff > maxSpread[tier] then
      return false
    end
  else
    if absYDiff > absXDiff or absXDiff > maxDist[tier] or absYDiff > maxSpread[tier] then
      return false
    end
  end
...
function Golf:getHoppableTargets(unit, startingPos, range)
    ...
    local targets = {}
    local leftTargets = self:getTargetsInDirection(unit, startingPos, {x = 1, y = 0}, range)
    local rightTargets = self:getTargetsInDirection(unit, startingPos, {x = -1, y = 0}, range)
    local upTargets = self:getTargetsInDirection(unit, startingPos, {x = 0, y = 1}, range)
    local downTargets = self:getTargetsInDirection(unit, startingPos, {x = 0, y = -1}, range)
...
function Golf:canExecuteWithTarget(unit, endPos, targetPos, strParam)  
    ...
    if not Golf.isInPreExecute then
        ...
        if not Golf.isInHops then
            ...
            local sourcePoint = Golf.destination
            if #Golf.hops > 0 then
                sourcePoint = Golf.hops[#Golf.hops]
            end
            
            local targets = self:getHoppableTargets(unit, sourcePoint, maxHopsRange[tier])
...
function Golf:preExecute(unit, targetPos, strParam, endPos)
    ...
    if tier == 2 then
        Wargroove.displayTarget(destination)
        
        Golf.isInHops = true
        for i = 1,maxHops[tier] do
            local sourcePoint = Golf.destination
            if #Golf.hops > 0 then
                sourcePoint = Golf.hops[#Golf.hops]
            end
            local hoppableTargets = self:getHoppableTargets(unit, sourcePoint, maxHopsRange[tier])
            if #hoppableTargets == 0 then
                break
            end
            ...
            local hop = Wargroove.getSelectedTarget()
            if hop ~= nil then
                Wargroove.displayTarget(hop)
                table.insert(Golf.hops, hop)
            else
    ...
    local hopsString = ""
    if #Golf.hops > 0 then
        hopsString = ";" .. self:buildTargetString(Golf.hops)
    end

    Wargroove.clearDisplayTargets()
    self:cleanUpPreExecute()

    return true, Golf.movingUnit .. ";" .. destination.x .. "," .. destination.y .. hopsString
end
...
function Golf:golfUnitToTarget(unit, targetUnit, teleportPosition, targetPos, path, tier, number)
    ...
    if (number-1 == maxHops[tier]) then
        local targetDamage = Combat:getGrooveAttackerDamage(unit, targetUnit, "average", targetUnit.pos, targetPos, path, nil) * damagePercent[tier]
        targetUnit:setHealth(targetUnit.health - targetDamage, unit.id)
    end

    if damageRange[tier] > 0 then
    ...
    -- Damage nearby units
    for i, pos in ipairs(Wargroove.getTargetsInRange(targetUnit.pos, damageRange[tier], "unit")) do
        local u = Wargroove.getUnitAt(pos)
        if u and u.id ~= targetUnit.id and Wargroove.areEnemies(u.playerId, unit.playerId) then
            local damage = Combat:getGrooveAttackerDamage(unit, u, "average", targetUnit.pos, pos, path, nil) * damagePercent[tier]
...
function Golf:generateOrders(unitId, canMove)
    ...
    for i, pos in pairs(movePositions) do
        ...
        local teleportPositionsInRange = Wargroove.getTargetsInRange(pos, maxDist[tier] * 2, "empty")
...
function Golf:getScore(unitId, order)
    ...
    local function computeDamageValue(u, target, scoreMult)
        local damage = Combat:getGrooveAttackerDamage(unit, u, "aiSimulation", teleportPosition, u.pos, path, nil) * damagePercent[tier]
