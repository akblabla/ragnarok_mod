...
function SpellHeal:canExecuteAnywhere(unit)
    return Wargroove.getMoney(unit.playerId) >= SpellHeal:calculateSpellCost(unit)
end

function SpellHeal:calculateSpellCost(unit)
    local discount = 1.0

    if Wargroove.isConquestMode() then 
        discount = unit.unitClass.verbCostMultiplier
    end
...
function SpellHeal:getCostAt(unit, endPos, targetPos)
    return SpellHeal:calculateSpellCost(unit)
end


function SpellHeal:execute(unit, targetPos, strParam, path)
    Wargroove.changeMoney(unit.playerId, -SpellHeal:calculateSpellCost(unit))
