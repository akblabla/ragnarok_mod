...
function RepositionVine:execute(unit, targetPos, strParam, path)
    ...
    if unit.unitClassId == "thorny_vine" then
        local commanderId = Wargroove.getUnitState(unit, "commanderId")
        local commanderUnit = Wargroove.getUnitById(tonumber(commanderId))
        local enemyPositions = Wargroove.getTargetsInRange(unit.pos, 1, "unit")
        ...
        for _, enemyPos in pairs(enemyPositions) do
            ...
            if enemy and Wargroove.areEnemies(enemy.playerId, unit.playerId) and enemy.canBeAttacked and enemy.unitClass.isAttackable and not enemy.unitClass.isStructure then
                local damage = Combat:getGrooveAttackerDamage(commanderUnit, enemy, "average", unit.pos, enemyPos, path, nil) * thornyDamage
