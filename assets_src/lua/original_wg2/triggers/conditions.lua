...
function Conditions.populate(dst)
    ...
    dst["item_presence"] = Conditions.itemPresence
    ...
    dst["unit_groove_tiered"] = Conditions.unitGrooveTiered
    dst["unit_visible"] = Conditions.unitVisible
    dst["unit_ambushed"] = Conditions.unitAmbushed
    ...
    dst["location_compare"] = Conditions.locationCompare
    dst["unit_verb_used"] = Conditions.unitVerbUsed
    dst["unit_groove_verb_used"] = Conditions.unitGrooveVerbUsed
    dst["unit_interactions_used"] = Conditions.unitInteractionUsed
    dst["interactions_target"] = Conditions.interactionsTarget
    dst["unit_item_presence"] = Conditions.unitItemPresence
    dst["check_player_counter"] = Conditions.checkPlayerCounter
    dst["check_conquest_unlock"] = Conditions.checkConquestUnlock
end
...
function Conditions.itemPresence(context)
    -- "Does {0} have {1} {2} of {3} at {4}?"
    local operator = context:getOperator(1)
    local value = context:getInteger(2)
    local items = context:gatherItems(0, 3, 4)

    return operator(#items, value)
end
...
function Conditions.unitGroove(context)
    ...
    for i, unit in ipairs(units) do
        if (unit.unitClass.maxGroove > 0) then
            local groove = Wargroove.getGroovePercentage(unit.grooveCharge, unit)
...
function Conditions.unitGrooveTiered(context)
    -- "Does {0} have {1} {2} of {3} at {4} with tier {5} groove charged?"
    local operator = context:getOperator(1)
    local value = context:getInteger(2)
    local units = context:gatherUnits(0, 3, 4)
    local value2 = context:getInteger(5)

    local nMatching = 0
    for i, unit in ipairs(units) do
        if (unit.unitClass.maxGroove > 0) then

            value2 = clamp(value2, 1, 2)

            local groove = Wargroove.getGroove(unit.grooveId)
            
            if unit.grooveCharge >= groove.grooveCost[value2] then
                nMatching = nMatching + 1
            end
        end
    end
...
function Conditions.unitVisible(context)
    -- "{4} {5} of {0} owned by {1} at {2} can be seen by {3}."
    local units = context:gatherUnits(1, 0, 2)
    local playerSeer = context:getPlayerId(3)

    local op = context:getOperator(4)
    local number = context:getInteger(5)
    ...
    return op(count, number)
end

function Conditions.unitAmbushed(context)
    -- "{0} owned by {1} at {2} has been ambushed."

    local units = context:gatherUnits(1, 0, 2)
    for i, unit in ipairs(units) do
        local ambushed = Wargroove.getUnitState(unit, "ambushed")
        if ambushed and ambushed == "true" then
            -- this resets after every check, so could lead to unexpected behaviour
            Wargroove.setUnitState(unit, "ambushed", "false")
            return true
        end
    end
...
function Conditions.locationCompare(context)
    -- "Is {0} {1} {2} (comparing tile positions)"
    local locA = context:getLocation(0)
    local locB = context:getLocation(2)
    local op = context:getLocationCompare(1)
    ...
    local result = context:boolOpLocations(locA, locB, function(a, b) return a and b end)
    
    if op == "overlaps" and #result > 0 then
        return true
    end
    if op == "doesnt_overlap" and #result == 0 then
        return true
    end
...
function Conditions.unitVerbUsed(context)
    -- "{0} owned by {1} at {2} used {3} targeting {4}"
    local units = context:gatherVerbsUsed(1, 0, 2, 3, 4)
    
    for i, unit in ipairs(units) do
        if not unit.verbUsed.isGroove then
            return true
        end
    end
...
function Conditions.unitGrooveVerbUsed(context)
    -- "{0} owned by {1} at {2} used {3} with {4} {5}% targeting {6}"
    local grooveOp = context:getOperator(4)
    local grooveValue = context:getInteger(5)

    local units = context:gatherVerbsUsed(1, 0, 2, 3, 6)

    for i, unit in ipairs(units) do
        if unit.verbUsed.isGroove then
            local groove = math.floor(unit.grooveChargeOnUse * 100 / unit.unitClass.maxGroove + 0.5)
            if grooveOp(groove, grooveValue) then
                return true
            end
        end
    end
...
function Conditions.unitInteractionUsed(context)
    -- "Has {0} owned by {1} at {2} used {4} targetting {3}?"
    local units = context:gatherInteractionsUsed(1, 0, 2, 3, 4)
...
function Conditions.interactionsTarget(context)
    -- "Is interactions target at {0}?"
    local loc = context:getLocation(0)

    local target = context.interactTargetPos
    if target == nil then
        return false
    end
    if loc == nil then
    ...
    local centre = findCentreOfLocation(loc)
    if centre.x == target.x and centre.y == target.y then
        return true
    end
...
function Conditions.unitItemPresence(context)
    -- "Does {0} have {1} with item {2} at {3}?"
    local units = context:gatherUnits(0, 1, 3)
    local itemClass = context:getItemClass(2)

    for _, unit in ipairs(units) do
        if unit.itemId == itemClass or ((unit.itemId ~= nil and unit.itemId ~= "") and itemClass == "*item") then
            return true
        end
    end
...
function Conditions.checkPlayerCounter(context)
     -- "Is player counter {0} {1} {2}?"
     local op = context:getOperator(1)
     local playerCounter = Wargroove.getPlayerCounter(context:getString(0))
     return op(playerCounter, context:getInteger(2))
end

function Conditions.checkConquestUnlock(context)
    -- "Triggers if the player has {0} unlocked."
    local unlockId = context:getString(0)
    return Wargroove.checkConquestUnlock(unlockId);
end
