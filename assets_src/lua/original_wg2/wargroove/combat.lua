...
function Combat:getBestWeapon(attacker, defender, delta, moved, facing)
	...
	local weapons = attacker.unitClass.weapons
		...
		if self:canUseWeapon(weapon, moved, delta, facing) then
			local dmg = Wargroove.getWeaponDamage(weapon, attacker, defender)
            if dmg > 0.0001 then
                return weapon, dmg
            end
        end
...
function Combat:solveRound(attacker, defender, solveType, isCounter, attackerPos, defenderPos, attackerPath)
	...
	local damage, hadPassive = self:getDamage(attacker, defender, solveType, isCounter, attackerPos, defenderPos, attackerPath, false, nil)	
	if (damage == nil) then
		return nil, false
	end
