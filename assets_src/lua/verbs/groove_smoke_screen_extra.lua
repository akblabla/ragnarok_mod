...
function SmokeScreen:getTier(unit)
    return 2
end
...
function SmokeScreen:canExecuteAnywhere(unit)
    local tier = self:getCurrentGrooveTier(unit)
    return tier>=SmokeScreen:getTier(unit)
end
...
function SmokeScreen:getSplashTargets(unit, targetPos, endPos)
    local tier = self:getCurrentGrooveTier(unit)
    local targets = Wargroove.getTargetsInRange(targetPos, 2, "all")
    return targets
end
...
function SmokeScreen:execute(unit, targetPos, strParam, path)
    ...
    table.insert(startingState, {key = "tier", value = tostring(2)})
